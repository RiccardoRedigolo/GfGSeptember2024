...
long long countWays(int n, std::string s){
  int size = n/2+1;
  std::vector<std::vector<std::vector<long long>>> table(size,std::vector<std::vector<long long>>(size,std::vector<long long>(2)));
...
int main(){
  std::string s = "T^F|F^T^T|F&F|F|F|F|F|F|F|F|F|F|F|F|F|F|F&T&T&T&T&T&T&T&T&T|F^F^T^F";
  int n = s.size();
